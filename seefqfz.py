...
if argquan != 3:
    print "This script requires one argument: the target directory"
    sys.exit(2)
...
lfqz=len(fqz) # gives number of elements in fqz: i.e. number of file in the PATH
...
FQRGX0=re.compile(r'.+\.fq')
FQRGX1=re.compile(r'.+\.fastq')
FQRGX2=re.compile(r'.+\.fq\.gz')
FQRGX3=re.compile(r'.+\.fastq\.gz')
